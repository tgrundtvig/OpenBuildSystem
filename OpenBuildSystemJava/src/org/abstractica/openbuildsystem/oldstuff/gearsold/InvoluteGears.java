package org.abstractica.openbuildsystem.oldstuff.gearsold;
...
public class InvoluteGears
{
	...
	public static Geometry2D rack2D(Core c, GearData gearData, double pitchHeight, double fitting)
	{
		...
		y -= pitchHeight - gearData.module;
		points.add(new Vector2(x, y));
		return c.polygon2D(points);
	}

	public static Geometry3D involuteGear3D(Core c, GearData gearData, double height, double fitting)
	{
		Geometry2D gear2D = involuteGear2D(c, gearData, fitting);
		return c.linearExtrude(gear2D, height, 0, 1, 2, 10);
	}

	public static Geometry2D involuteGear2D(Core c, GearData gearData, double fitting)
	{
		Group2D union = c.union2D();
		InvoluteTooth tooth = involuteTooth2D(c, gearData, fitting);
		...
		for(int i = 0; i < gearData.numberOfTeeth; ++i)
		{
			...
			center.add(nextLeft);
			Group2D rot = c.rotate2D(Math.toDegrees(i * gearData.pitchAngle));
			rot.add(tooth.toothGeometry);
			union.add(rot);
		}
		Geometry2D centerGeometry = c.polygon2D(center);
		union.add(centerGeometry);
		return union;
	}

	public static InvoluteTooth involuteTooth2D(Core c, GearData gearData, double fitting)
	{
		//Cutout
		...
		Geometry2D halfTooth = c.polygon2D(halfToothPoints);
		Group2D diff = c.difference2D();
		diff.add(halfTooth);
		diff.add(c.polygon2D(cutoutPoints));
		Group2D mirror = c.mirror2D(0,1);
		mirror.add(diff);
		Group2D union = c.union2D();
		union.add(mirror);
		union.add(diff);
		InvoluteTooth res = new InvoluteTooth(union, polarToCartesian(polarRight), polarToCartesian(polarLeft));
		return res;
	}
